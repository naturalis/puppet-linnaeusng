# Linnaeus Loadbalancer / reverse proxy
#
#
#
#
#
class role_linnaeusng::lb (


# use certificates, place certificates in files folder of puppet manifest. Be aware of the .gitignore so certs don't end up on github!
  Boolean $usecerthash     = true,
  Hash $cert_hash       = { 'STAR_linnaeus_naturalis_nl'    => { 'private' => 'STAR_linnaeus_naturalis_nl.key',
                                                            'cert'    => 'STAR_linnaeus_naturalis_nl.pem',
                                                            'cacert'  => 'STAR_linnaeus_naturalis_nl_cacert.pem' },
                       'STAR_naturalis_nl'             => { 'private' => 'STAR_naturalis_nl.key',
                                                            'cert'    => 'STAR_naturalis_nl.pem',
                                                            'cacert'  => 'STAR_naturalis_nl_cacert.pem' },
                       'www_nederlandsesoorten_nl'     => { 'private' => 'www_nederlandsesoorten_nl.key',
                                                            'cert'    => 'www_nederlandsesoorten_nl.pem',
                                                            'cacert'  => 'comodo_cacert_bundle.pem' },
                       'www_grasshoppersofeurope_com'  => { 'private' => 'www_grasshoppersofeurope_com.key',
                                                            'cert'    => 'www_grasshoppersofeurope_com.pem' },
                       'www_dutchcaribbeanspecies_org' => { 'private' => 'www_dutchcaribbeanspecies_org.key',
                                                            'cert'    => 'www_dutchcaribbeanspecies_org.pem' }
                     },
  Boolean $useletsencrypt       = false,
  Hash $letsencrypt_hash        = { 'determineren' => { 'letsencrypt_domains' => ['determineren.nederlandsesoorten.nl','determinatie.nederlandsesoorten.nl']},
                                    'nsr' => { 'letsencrypt_domains' => ['nederlandsesoorten.nl','www.nederlandsesoorten.nl','soortenregister.nl','www.soortenregister.nl']},
                                    'dcr' => { 'letsencrypt_domains' => ['dutchcaribbeanspecies.org','www.dutchcaribbeanspecies.org']},
                                  },
  $letsencrypt_email            = 'aut@naturalis.nl',
  $letsencrypt_version          = 'master',
  $letsencrypt_server           = 'https://acme-v02.api.letsencrypt.org/directory',
  Hash $server       = { 'default_server'            => { 'proxy' => 'http://default_server',
                                                    'server_name'  => ['_'],
                                                    'ssl'          => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_naturalis_nl.key',
                                                    'location_cfg_append' => { 'rewrite' => '^ http://linnaeus.naturalis.nl permanent'},
                                                   },
                   'nederlandsesoorten_alt'    => { 'proxy' => 'http://nsr-frontend',
                                                    'server_name'  => ['soortenregister.nl','www.soortenregister.nl','nederlandsesoorten.nl'],
                                                    'ssl'          => true,
#                                                    'ssl_cert'     => '/etc/ssl/certs/www_nederlandsesoorten_nl.pem',
#                                                    'ssl_key'      => '/etc/ssl/private/www_nederlandsesoorten_nl.key',
                                                    'ssl_cert'     => '/etc/letsencrypt/live/nederlandsesoorten.nl/cert.pem',
                                                    'ssl_key'      => '/etc/letsencrypt/live/nederlandsesoorten.nl/privkey.pem',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://www.nederlandsesoorten.nl$request_uri permanent'},
                                                   },
                   'nederlandsesoorten_server'  => { 'proxy' => 'http://nsr-frontend',
                                                    'server_name'  => ['www.nederlandsesoorten.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/letsencrypt/live/nederlandsesoorten.nl/cert.pem',
                                                    'ssl_key'      => '/etc/letsencrypt/live/nederlandsesoorten.nl/privkey.pem',
#                                                    'ssl_cert'     => '/etc/ssl/certs/www_nederlandsesoorten_nl.pem',
#                                                    'ssl_key'      => '/etc/ssl/private/www_nederlandsesoorten_nl.key',
                                                   },
                   'grasshoppers_alt'          => { 'proxy' => 'http://grasshoppers-frontend',
                                                    'server_name'  => ['www.ortheur.org','ortheur.org','grasshoppersofeurope.com'],
                                                    'ssl'          => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/www_grasshoppersofeurope_com.pem',
                                                    'ssl_key'      => '/etc/ssl/private/www_grasshoppersofeurope_com.key',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://www.grasshoppersofeurope.com$request_uri permanent'},
                                                   },
                   'grasshoppers_server'        => { 'proxy' => 'http://grasshoppers-frontend',
                                                    'server_name'  => ['www.grasshoppersofeurope.com'],
                                                    'ssl'          => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/www_grasshoppersofeurope_com.pem',
                                                    'ssl_key'      => '/etc/ssl/private/www_grasshoppersofeurope_com.key',
                                                   },
                   'dutchcaribbeanspecies_server' => { 'proxy' => 'http://dcs-frontend',
                                                    'server_name'  => ['www.dutchcaribbeanspecies.org','dutchcaribbeanspecies.org'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/letsencrypt/live/dutchcaribbeanspecies.org/cert.pem',
                                                    'ssl_key'      => '/etc/letsencrypt/live/dutchcaribbeanspecies.org/privkey.pem',
                                                   },
                   'various-001_server'         => { 'proxy' => 'http://various-001',
                                                    'server_name'  => ['various-001.linnaeus.naturalis.nl','aetideidae.linnaeus.naturalis.nl','agromyzidae.linnaeus.naturalis.nl','bats-india.linnaeus.naturalis.nl','bris.linnaeus.naturalis.nl','birds-europe.linnaeus.naturalis.nl','blackflies.linnaeus.naturalis.nl','braconidae.linnaeus.naturalis.nl','butterflies-europe.linnaeus.naturalis.nl','chironomidae-exuviae.linnaeus.naturalis.nl','chironomidae-larvae.linnaeus.naturalis.nl','crabs-japan.linnaeus.naturalis.nl','dagvlinders-europa.linnaeus.naturalis.nl','davalliacea.linnaeus.naturalis.nl','duikgids.linnaeus.naturalis.nl','paddenstoelen.linnaeus.naturalis.nl','decapodos-chile.linnaeus.naturalis.nl','diaspididae.linnaeus.naturalis.nl','eurasian-tortricidae.linnaeus.naturalis.nl','european-limnofauna.linnaeus.naturalis.nl','fish-eggs-and-larvae-from-amw.linnaeus.naturalis.nl','fishes-fnam.linnaeus.naturalis.nl','flora-arborea-chile.linnaeus.naturalis.nl','caesalpinioideae.linnaeus.naturalis.nl','flora-burren.linnaeus.naturalis.nl','fw-oligochaeta.linnaeus.naturalis.nl','dinoflagellates.linnaeus.naturalis.nl','insecten-europa.linnaeus.naturalis.nl','flora-british-isles.linnaeus.naturalis.nl','caribbean-diving.linnaeus.naturalis.nl','mushrooms.linnaeus.naturalis.nl','lemurs.linnaeus.naturalis.nl','libellenlarven.linnaeus.naturalis.nl','ns-anthozoa.linnaeus.naturalis.nl','ns-brachiopoda.linnaeus.naturalis.nl','ns-crustacea.linnaeus.naturalis.nl','ns-echinodermata.linnaeus.naturalis.nl','ns-miscellaneous-worms.linnaeus.naturalis.nl','ns-mollusca.linnaeus.naturalis.nl','ns-nemertina.linnaeus.naturalis.nl','ns-platyhelminthes.linnaeus.naturalis.nl','ns-polychaeta.linnaeus.naturalis.nl','ns-pycnogonida.linnaeus.naturalis.nl','ns-sipuncula.linnaeus.naturalis.nl','ns-tunicata.linnaeus.naturalis.nl','marine-lobsters.linnaeus.naturalis.nl','marine-mammals.linnaeus.naturalis.nl','marine-planarians.linnaeus.naturalis.nl','marine-ostracods.linnaeus.naturalis.nl','mormyridae.linnaeus.naturalis.nl','sea-cucumbers.linnaeus.naturalis.nl','orchids-philippines.linnaeus.naturalis.nl','otoliths-northsea.linnaeus.naturalis.nl','pelagic-molluscs.linnaeus.naturalis.nl','prosea-bamboos.linnaeus.naturalis.nl','prosea-edible-fruits-and-nuts.linnaeus.naturalis.nl','prosea-rattans.linnaeus.naturalis.nl','prosea-timber-trees.linnaeus.naturalis.nl','pteromalus.linnaeus.naturalis.nl','indo-malayan-reef-corals.linnaeus.naturalis.nl','reptielen-en-amfibieen.linnaeus.naturalis.nl','reptiles-and-amphibians.linnaeus.naturalis.nl','sharks.linnaeus.naturalis.nl','sponges-ne-atlantic.linnaeus.naturalis.nl','torymus.linnaeus.naturalis.nl','turbellaria.linnaeus.naturalis.nl','turtles.linnaeus.naturalis.nl','genitalia-preparation.linnaeus.naturalis.nl','useful-plants.linnaeus.naturalis.nl','vogels-europa.linnaeus.naturalis.nl','zoetwatervissen.linnaeus.naturalis.nl','ns-zooplankton.linnaeus.naturalis.nl','sat-zooplankton.linnaeus.naturalis.nl','legacy.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-002_server'         => { 'proxy' => 'http://various-002',
                                                    'server_name' => ['various-002.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-003_server'         => { 'proxy' => 'http://various-003',
                                                    'server_name' => ['various-003.linnaeus.naturalis.nl','img-classify.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-004_server'         => { 'proxy' => 'http://various-004',
                                                    'server_name' => ['various-004.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-005_server'         => { 'proxy' => 'http://various-005',
                                                    'server_name' => ['various-005.linnaeus.naturalis.nl','doignies.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-006_server'         => { 'proxy' => 'http://various-006',
                                                    'server_name' => ['various-006.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-007_server'         => { 'proxy' => 'http://various-007',
                                                    'server_name' => ['various-007.linnaeus.naturalis.nl','glomera.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-008_server'         => { 'proxy' => 'http://various-008',
                                                    'server_name' => ['various-008.linnaeus.naturalis.nl','flora-central-africa.linnaeus.naturalis.nl','grasses-cameroon.linnaeus.naturalis.nl','grasses-africa.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-010_server'         => { 'proxy' => 'http://various-010',
                                                    'server_name' => ['various-010.linnaeus.naturalis.nl','torymus.linnaeus.naturalis.nl','pteromalus.linnaeus.naturalis.nl','tetrastichinae.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-011_server'         => { 'proxy' => 'http://various-011',
                                                    'server_name' => ['various-011.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'identify_server'            => { 'proxy' => 'http://various-011',
                                                    'server_name' => ['identify.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_naturalis_nl.key',
                                                   },
                   'various-012_server'         => { 'proxy' => 'http://various-012',
                                                    'server_name' => ['various-012.linnaeus.naturalis.nl','orchidsnguinea.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-014_server'         => { 'proxy' => 'http://various-014',
                                                    'server_name' => ['various-014.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'orchidsnguinea_server'      => { 'proxy' => 'http://various-012',
                                                    'server_name' => ['orchids.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_naturalis_nl.key',
                                                   },
                   'various-016_server'         => { 'proxy' => 'http://various-016',
                                                    'server_name' => ['various-016.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'heukels_server'             => { 'proxy' => 'http://various-016',
                                                    'server_name' => ['heukels.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_naturalis_nl.key',
                                                   },
                   'various-017_server'         => { 'proxy' => 'http://various-017',
                                                    'server_name' => ['various-017.linnaeus.naturalis.nl','flore-de-la-belgique.linnaeus.naturalis.nl','flora-van-belgie.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-018_server'         => { 'proxy' => 'http://various-018',
                                                    'server_name' => ['various-018.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-019_server'         => { 'proxy' => 'http://various-019',
                                                    'server_name' => ['various-019.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-020_server'         => { 'proxy' => 'http://various-020',
                                                    'server_name' => ['various-020.linnaeus.naturalis.nl','catpalhet.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-021_server'         => { 'proxy' => 'http://various-021',
                                                    'server_name' => ['various-021.linnaeus.naturalis.nl','worldferns.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-022_server'         => { 'proxy' => 'http://various-022',
                                                    'server_name' => ['various-022.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-023_server'         => { 'proxy' => 'http://various-023',
                                                    'server_name' => ['various-023.linnaeus.naturalis.nl','idredsea.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-024_server'         => { 'proxy' => 'http://various-024',
                                                    'server_name' => ['various-024.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-025_server'         => { 'proxy' => 'http://various-025',
                                                    'server_name' => ['various-025.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-026_server'         => { 'proxy' => 'http://various-026',
                                                    'server_name' => ['various-026.linnaeus.naturalis.nl','hermannceylon.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-027_server'         => { 'proxy' => 'http://various-027',
                                                    'server_name' => ['various-027.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-028_server'         => { 'proxy' => 'http://various-028',
                                                    'server_name' => ['various-028.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-029_server'         => { 'proxy' => 'http://various-029',
                                                    'server_name' => ['various-029.linnaeus.naturalis.nl','cetaceanscapulae.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-030_server'         => { 'proxy' => 'http://various-030',
                                                    'server_name' => ['various-030.linnaeus.naturalis.nl','viva.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-031_server'         => { 'proxy' => 'http://various-031',
                                                    'server_name' => ['various-031.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-032_server'         => { 'proxy' => 'http://various-032',
                                                    'server_name' => ['www.dierenzoeker.nl','dierenzoeker.nl','www.dierezoeker.nl','dierezoeker.nl','various-032.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-033_server'         => { 'proxy' => 'http://various-033',
                                                    'server_name' => ['various-033.linnaeus.naturalis.nl','plantgenera.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-034_server'         => { 'proxy' => 'http://various-034',
                                                    'server_name' => ['various-034.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-035_server'         => { 'proxy' => 'http://various-035',
                                                    'server_name' => ['various-035.linnaeus.naturalis.nl','oggngbotany.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-036_server'         => { 'proxy' => 'http://various-036',
                                                    'server_name' => ['various-036.linnaeus.naturalis.nl','SEAsianfloweringplants.linnaeus.naturalis.nl'],
                                                    'ssl'         => true,
                                                    'ssl_cert'    => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'     => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-037_server'         => { 'proxy' => 'http://various-037',
                                                    'server_name'  => ['various-037.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-038_server'         => { 'proxy' => 'http://various-038',
                                                    'server_name'  => ['various-038.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-039_server'         => { 'proxy' => 'http://various-039',
                                                    'server_name'  => ['various-039.linnaeus.naturalis.nl','euphausiids.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'determineren_server'        => { 'proxy' => 'http://various-040',
                                                    'server_name'  => ['determineren.nederlandsesoorten.nl','determinatie.nederlandsesoorten.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/letsencrypt/live/determineren.nederlandsesoorten.nl/cert.pem',
                                                    'ssl_key'      => '/etc/letsencrypt/live/determineren.nederlandsesoorten.nl/privkey.pem',
                                                   },
                   'various-040_server'         => { 'proxy' => 'http://various-040',
                                                    'server_name'  => ['various-040.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-041_server'         => { 'proxy' => 'http://various-041',
                                                    'server_name'  => ['various-041.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-042_server'         => { 'proxy' => 'http://various-042',
                                                    'server_name'  => ['various-042.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-043_server'         => { 'proxy' => 'http://various-043',
                                                    'server_name'  => ['various-043.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-044_server'         => { 'proxy' => 'http://various-044',
                                                    'server_name'  => ['various-044.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-045_server'         => { 'proxy' => 'http://various-045',
                                                    'server_name'  => ['various-045.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-046_server'         => { 'proxy' => 'http://various-046',
                                                    'server_name'  => ['various-046.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-047_server'         => { 'proxy' => 'http://various-047',
                                                    'server_name'  => ['various-047.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-048_server'         => { 'proxy' => 'http://various-048',
                                                    'server_name'  => ['various-048.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-049_server'         => { 'proxy' => 'http://various-049',
                                                    'server_name'  => ['various-049.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-050_server'         => { 'proxy' => 'http://various-050',
                                                    'server_name'  => ['various-050.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-051_server'         => { 'proxy' => 'http://various-051',
                                                    'server_name'  => ['various-051.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-052_server'         => { 'proxy' => 'http://various-052',
                                                    'server_name'  => ['various-052.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-053_server'         => { 'proxy' => 'http://various-053',
                                                    'server_name'  => ['various-053.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-054_server'         => { 'proxy' => 'http://various-054',
                                                    'server_name'  => ['various-054.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-055_server'         => { 'proxy' => 'http://various-055',
                                                    'server_name'  => ['various-055.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-056_server'         => { 'proxy' => 'http://various-056',
                                                    'server_name'  => ['various-056.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-057_server'         => { 'proxy' => 'http://various-057',
                                                    'server_name'  => ['various-057.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-058_server'         => { 'proxy' => 'http://various-058',
                                                    'server_name'  => ['various-058.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-059_server'         => { 'proxy' => 'http://various-059',
                                                    'server_name'  => ['various-059.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                   'various-060_server'         => { 'proxy' => 'http://various-060',
                                                    'server_name'  => ['various-060.linnaeus.naturalis.nl'],
                                                    'ssl'          => true,
                                                    'ssl_redirect' => true,
                                                    'ssl_cert'     => '/etc/ssl/certs/STAR_linnaeus_naturalis_nl.pem',
                                                    'ssl_key'      => '/etc/ssl/private/STAR_linnaeus_naturalis_nl.key',
                                                   },
                 },
  Hash $location    = { 'loc_orchidsnguinea_ssl'    => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'orchidsnguinea_server',
                                                    'proxy' => 'https://various-012'},
                   'loc_various-002_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-002_server',
                                                    'proxy' => 'https://various-002'},
                   'loc_various-003_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-003_server',
                                                    'proxy' => 'https://various-003'},
                   'loc_various-004_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-004_server',
                                                    'proxy' => 'https://various-004'},
                   'loc_various-005_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-005_server',
                                                    'proxy' => 'https://various-005'},
                   'loc_various-006_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-006_server',
                                                    'proxy' => 'https://various-006'},
                   'loc_various-007_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-007_server',
                                                    'proxy' => 'https://various-007'},
                   'loc_various-008_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-008_server',
                                                    'proxy' => 'https://various-008'},
                   'loc_various-010_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-010_server',
                                                    'proxy' => 'https://various-010'},
                   'loc_various-011_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-011_server',
                                                    'proxy' => 'https://various-011'},
                   'loc_identify_ssl'          => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'identify_server',
                                                    'proxy' => 'https://various-011'},
                   'loc_various-012_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-012_server',
                                                    'proxy' => 'https://various-012'},
                   'loc_various-014_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-014_server',
                                                    'proxy' => 'https://various-014'},
                   'loc_various-016_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-016_server',
                                                    'proxy' => 'https://various-016'},
                   'loc_various-017_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-017_server',
                                                    'proxy' => 'https://various-017'},
                   'loc_various-018_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-018_server',
                                                    'proxy' => 'https://various-018'},
                   'loc_various-019_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-019_server',
                                                    'proxy' => 'https://various-019'},
                   'loc_various-020_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-020_server',
                                                    'proxy' => 'https://various-020'},
                   'loc_various-021_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-021_server',
                                                    'proxy' => 'https://various-021'},
                   'loc_various-022_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-022_server',
                                                    'proxy' => 'https://various-022'},
                   'loc_various-023_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-023_server',
                                                    'proxy' => 'https://various-023'},
                   'loc_various-024_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-024_server',
                                                    'proxy' => 'https://various-024'},
                   'loc_various-025_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-025_server',
                                                    'proxy' => 'https://various-025'},
                   'loc_various-026_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-026_server',
                                                    'proxy' => 'https://various-026'},
                   'loc_various-027_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-027_server',
                                                    'proxy' => 'https://various-027'},
                   'loc_various-028_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-028_server',
                                                    'proxy' => 'https://various-028'},
                   'loc_various-029_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-029_server',
                                                    'proxy' => 'https://various-029'},
                   'loc_various-030_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-030_server',
                                                    'proxy' => 'https://various-030'},
                   'loc_various-031_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-031_server',
                                                    'proxy' => 'https://various-031'},
                   'loc_various-032_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-032_server',
                                                    'proxy' => 'https://various-032'},
                   'loc_various-033_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-033_server',
                                                    'proxy' => 'https://various-033'},
                   'loc_various-034_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-034_server',
                                                    'proxy' => 'https://various-034'},
                   'loc_various-035_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-035_server',
                                                    'proxy' => 'https://various-035'},
                   'loc_various-036_ssl'       => { 'location' => '/linnaeus_ng/admin/',
                                                    'location_cfg_append' => { 'rewrite' => '^ https://$host$request_uri? permanent' },
                                                    'server' => 'various-036_server',
                                                    'proxy' => 'https://various-036'},
                 },
  Hash $upstream    = { 'various-001'               => { 'members' => ['172.16.1.160'] },
                   'various-002'               => { 'members' => ['172.16.1.54'] },
                   'various-003'               => { 'members' => ['172.16.1.55'] },
                   'various-004'               => { 'members' => ['172.16.1.56'] },
                   'various-005'               => { 'members' => ['172.16.1.57'] },
                   'various-006'               => { 'members' => ['172.16.1.58'] },
                   'various-007'               => { 'members' => ['172.16.1.59'] },
                   'various-008'               => { 'members' => ['172.16.1.60'] },
                   'various-010'               => { 'members' => ['172.16.1.62'] },
                   'various-011'               => { 'members' => ['172.16.1.63'] },
                   'various-012'               => { 'members' => ['172.16.1.64'] },
                   'various-014'               => { 'members' => ['172.16.1.66'] },
                   'various-016'               => { 'members' => ['172.16.1.75'] },
                   'various-017'               => { 'members' => ['172.16.1.79'] },
                   'various-018'               => { 'members' => ['172.16.1.76'] },
                   'various-019'               => { 'members' => ['172.16.1.77'] },
                   'various-020'               => { 'members' => ['172.16.1.78'] },
                   'various-021'               => { 'members' => ['172.16.1.80'] },
                   'various-022'               => { 'members' => ['172.16.1.83'] },
                   'various-023'               => { 'members' => ['172.16.1.81'] },
                   'various-024'               => { 'members' => ['172.16.1.82'] },
                   'various-025'               => { 'members' => ['172.16.1.84'] },
                   'various-026'               => { 'members' => ['172.16.1.109'] },
                   'various-027'               => { 'members' => ['172.16.1.96'] },
                   'various-028'               => { 'members' => ['172.16.1.97'] },
                   'various-029'               => { 'members' => ['172.16.1.98'] },
                   'various-030'               => { 'members' => ['172.16.1.110'] },
                   'various-031'               => { 'members' => ['172.16.1.100'] },
                   'various-032'               => { 'members' => ['172.16.1.101'] },
                   'various-033'               => { 'members' => ['172.16.1.102'] },
                   'various-034'               => { 'members' => ['172.16.1.103'] },
                   'various-035'               => { 'members' => ['172.16.1.104'] },
                   'various-036'               => { 'members' => ['172.16.1.145'] },
                   'various-037'               => { 'members' => ['172.16.1.157'] },
                   'various-038'               => { 'members' => ['172.16.1.158'] },
                   'various-039'               => { 'members' => ['172.16.1.159'] },
                   'various-040'               => { 'members' => ['172.16.1.155'] },
                   'various-041'               => { 'members' => ['172.16.1.167'] },
                   'various-042'               => { 'members' => ['172.16.1.162'] },
                   'various-043'               => { 'members' => ['172.16.1.164'] },
                   'various-044'               => { 'members' => ['172.16.1.165'] },
                   'various-045'               => { 'members' => ['172.16.1.166'] },
                   'various-046'               => { 'members' => ['172.16.1.168'] },
                   'various-047'               => { 'members' => ['172.16.1.169'] },
                   'various-048'               => { 'members' => ['172.16.1.170'] },
                   'various-049'               => { 'members' => ['172.16.1.171'] },
                   'various-050'               => { 'members' => ['172.16.1.172'] },
                   'various-051'               => { 'members' => ['172.16.1.185'] },
                   'various-052'               => { 'members' => ['172.16.1.175'] },
                   'various-053'               => { 'members' => ['172.16.1.176'] },
                   'various-054'               => { 'members' => ['172.16.1.177'] },
                   'various-055'               => { 'members' => ['172.16.1.178'] },
                   'various-056'               => { 'members' => ['172.16.1.179'] },
                   'various-057'               => { 'members' => ['172.16.1.181'] },
                   'various-058'               => { 'members' => ['172.16.1.182'] },
                   'various-059'               => { 'members' => ['172.16.1.183'] },
                   'various-060'               => { 'members' => ['172.16.1.184'] },
                   'nsr-frontend'              => { 'members' => ['172.16.1.89'] },
                   'dcs-frontend'              => { 'members' => ['172.16.1.128'] },
                   'grasshoppers-frontend'     => { 'members' => ['145.136.243.126'] },
                   'default_server'            => { 'members' => ['145.136.241.149'] },
                 },

){

# configure SSL 
  if $usecerthash == true {
    create_resources('role_linnaeusng::lbcerts', $cert_hash)
  }

# letsencrypt
  if $useletsencrypt == true {
  # install letsencrypt certs only and crontab
    class { ::letsencrypt:
      repo           => 'https://github.com/certbot/certbot.git',
      install_method => 'vcs',
      version        => 'master',
      config         => {
        email  => $role_linnaeusng::lb::letsencrypt_email,
        server => $role_linnaeusng::lb::letsencrypt_server,
      }
    }
    create_resources('role_linnaeusng::lbssl', $letsencrypt_hash,{})
  }

class { 'nginx':
  names_hash_bucket_size => '512',
  client_max_body_size   => '100M'
}

#  create_resources
  create_resources(nginx::resource::server,$server,{})
  create_resources(nginx::resource::location,$location,{})
  create_resources(nginx::resource::upstream,$upstream,{})
}

